...
class LP_Collapsing_UP_GS_multi(torch.nn.Module):
    def __init__(self, pyramid_level, align_corners):
        super(LP_Collapsing_UP_GS_multi, self).__init__()
    ...
    def Collapse_grid(self, LP_pyr, pMtrx, pyr_size, align_corners=True):
        ...
        # Grid Sampling
        for i in range(len(LP_pyr)):
            imageWarp = torch.nn.functional.grid_sample(LP_pyr[i], grid_list[i], 'bilinear',align_corners=align_corners)
